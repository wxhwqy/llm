...
struct CustomFP8E4M3 {
    uint8_t _v;
};
typedef struct CustomFP8E4M3 fp8e4m3_t;
...
TypeTo cast(TypeFrom val) {
    ...
    } else if constexpr (std::is_same<TypeTo, fp8e4m3_t>::value && std::is_same<TypeFrom, float>::value) {
        return _f32_to_fp8e4m3(val);
    } else if constexpr (std::is_same<TypeTo, fp8e4m3_t>::value && !std::is_same<TypeFrom, float>::value) {
        return _f32_to_fp8e4m3(static_cast<float>(val));
    } else if constexpr (std::is_same<TypeFrom, fp8e4m3_t>::value && std::is_same<TypeTo, float>::value) {
        return _fp8e4m3_to_f32(val);
    } else if constexpr (std::is_same<TypeFrom, fp8e4m3_t>::value && !std::is_same<TypeTo, float>::value) {
        return static_cast<TypeTo>(_fp8e4m3_to_f32(val));
    } else {
